...
async function getRawValue(key) {
  if (METRICS_DISABLED) return null;
  ...
  try {
    ...
    console.warn(`Metrics getRawValue failed for ${key}:`, err?.message || err);
    return null;
  }
}

async function setRawValue(key, value, ttlSeconds) {
  if (METRICS_DISABLED) return false;
  if (!UPSTASH_URL || !UPSTASH_TOKEN) return false;
  try {
    const command = ["SET", key, String(value)];
    if (Number.isFinite(ttlSeconds) && ttlSeconds > 0) {
      command.push("EX", Math.max(1, Math.floor(ttlSeconds)));
    }
    const res = await fetch(`${UPSTASH_URL}/pipeline`, {
      method: "POST",
      headers: {
    ...
    console.warn(`Metrics setRawValue failed for ${key}:`, err?.message || err);
    return false;
  }
}
...
async function deleteRawValue(key) {
  ...
  try {
    ...
    console.warn(
      `Metrics deleteRawValue failed for ${key}:`,
      err?.message || err,
    );
    return false;
  }
