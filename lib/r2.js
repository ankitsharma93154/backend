...
// DIAGNOSTIC (Jul 2026): keepAlive tuning didn't move the numbers at all,
// which points at a different theory — concurrent requests may be landing
// on separate function instances that each get a fresh module load (and
// therefore a fresh agent + empty connection pool), so there's never a
// warm socket to reuse in the first place. instanceId is generated once
// per module load (i.e. once per cold instance); rising callCount on a
// repeated instanceId in the logs proves reuse is possible; a different
// instanceId on every single request proves it isn't.
...
const r2 = new S3Client({
  ...
  requestHandler: new NodeHttpHandler({
    ...
    httpsAgent: new https.Agent({
      keepAlive: true,
      keepAliveMsecs: 60000, // survive up to 60s idle between requests on the same warm instance
      maxSockets: 10,
    }),
  }),
});
...
async function getFromR2(key) {
  ...
  try {
    ...
    waitUntil(recordR2Get("hit").catch(() => {}));
    return data;
  } catch (err) {
