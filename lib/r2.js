...
const { recordR2Get, recordR2Put } = require("./metrics");

// ===== Fail loud at startup instead of failing mysteriously at request time =====
// Without this, a missing/misspelled env var (e.g. in your host's project
// settings) just becomes "https://undefined.r2.cloudflarestorage.com" — and
// the actual error surfaces later as a confusing DNS/connection failure deep
// in the SDK.
...
// DIAGNOSTIC (Jul 2026): keepAlive tuning didn't move the numbers at all on
// Vercel, which pointed at concurrent requests landing on separate function
// instances each getting a fresh module load. On App Platform this is much
// less of a concern since the process is long-lived rather than
// spun-up-per-request — connection reuse across requests within the same
// process should behave more predictably here. instanceId is generated once
// per process start; a stable instanceId across requests confirms reuse.
...
const r2 = new S3Client({
  ...
  // Fail fast instead of hanging rather than let a slow/unreachable R2
  // endpoint eat into request latency before our own try/catch fallback
  // logic gets a chance to kick in.
  maxAttempts: 2,
  requestHandler: new NodeHttpHandler({
    ...
    // sockets don't survive long enough to go stale — which is what
    // caused the original "SSL alert number 40 / handshake failure" bug
    // that led to disabling it. maxSockets caps how many concurrent
    // connections one process holds open, rather than unbounded growth
    // under concurrent requests.
    httpsAgent: new https.Agent({
      keepAlive: true,
      keepAliveMsecs: 60000, // survive up to 60s idle between requests
      maxSockets: 10,
    }),
  }),
});
...
async function getFromR2(key) {
  ...
  try {
    ...
    console.log(
      `[R2-GET] hit  instance=${instanceId} call#${callNum} key=${key} bytes=${text.length} dur=${Date.now() - start}ms`,
    );
    // No waitUntil needed — App Platform runs a persistent process, so
    // this just completes in the background naturally.
    recordR2Get("hit").catch(() => {});
    return data;
  } catch (err) {
    const dur = Date.now() - start;
    if (err?.name === "NoSuchKey") {
      console.log(
        `[R2-GET] miss instance=${instanceId} call#${callNum} key=${key} dur=${dur}ms`,
      );
      recordR2Get("miss").catch(() => {});
    } else {
      console.warn(
        `[R2-GET] fail instance=${instanceId} call#${callNum} key=${key} dur=${dur}ms err=${err?.message || err}`,
      );
      recordR2Get("fail").catch(() => {});
    }
    return null;
  }
}
...
async function saveToR2(key, data) {
  ...
  try {
    ...
    console.log(
      `[R2-PUT] ok   instance=${instanceId} call#${callNum} key=${key} bytes=${body.length} dur=${Date.now() - start}ms`,
    );
    recordR2Put("ok").catch(() => {});
    return true;
  } catch (err) {
    console.error(
      `[R2-PUT] fail instance=${instanceId} call#${callNum} key=${key} bytes=${body.length} dur=${Date.now() - start}ms err=${err?.message || err}`,
    );
    recordR2Put("fail").catch(() => {});
    return false;
  }
}
