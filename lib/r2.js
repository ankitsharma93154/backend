...
async function getFromR2(key) {
  ...
  try {
    ...
    console.log(
      `[R2-GET] hit  instance=${instanceId} call#${callNum} key=${key} dur=${Date.now() - start}ms`,
    );
    return data;
  } catch (err) {
...
async function saveToR2(key, data) {
  ...
  try {
    ...
    console.log(
      `[R2-PUT] ok   instance=${instanceId} call#${callNum} key=${key} dur=${Date.now() - start}ms`,
    );
    return true;
  } catch (err) {
    console.error(
      `[R2-PUT] fail instance=${instanceId} call#${callNum} key=${key} dur=${Date.now() - start}ms err=${err?.message || err}`,
    );
    return false;
  }
}
